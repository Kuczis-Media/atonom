...
function energySliderToCoeff(value) {
...
function cameraHeightSliderToPosition(value) {
    var numericValue = parseFloat(value);
    if (isNaN(numericValue)) {
        return 0;
    }
    return (numericValue - 500) / 2;
}

function cameraDistanceSliderToUnits(value) {
    var numericValue = parseFloat(value);
    if (isNaN(numericValue)) {
        return 3;
    }
    return Math.max(0.5, numericValue / 100.0);
}
...
function cameraDistanceToSliderValue(distance) {
    return clampSliderValue(Math.round(distance * 100));
}
...
$(document).ready(function () {
    ...
    Physics(function (world) {
        ...
        var $dragRange = $("#dragRange");
        var $cameraHeight = $("#cameraHeight");
        var $cameraDistance = $("#cameraDistance");

        var drag = Physics.behavior('parasitic-drag', { coeff: energySliderToCoeff($dragRange.val()) });
        world.add(drag);

        var applyEnergyFromSlider = function (value) {
            var coeff = energySliderToCoeff(value);
            drag.options.coeff = coeff;
            $('#dragRangeValue').text(coeff.toFixed(4));
        };

        var applyCameraHeightFromSlider = function (value) {
            gCameraHeight = cameraHeightSliderToPosition(value);
            $('#cameraHeightValue').text(gCameraHeight.toFixed(0));
        };

        var applyCameraDistanceFromSlider = function (value) {
            gCameraDistance = cameraDistanceSliderToUnits(value);
            $('#cameraDistanceValue').text(gCameraDistance.toFixed(1));
        };

        applyEnergyFromSlider($dragRange.val());
        applyCameraHeightFromSlider($cameraHeight.val());
        applyCameraDistanceFromSlider($cameraDistance.val());

        $dragRange.on('input change', function () {
            applyEnergyFromSlider(this.value);
        });

        $cameraHeight.on('input change', function () {
            applyCameraHeightFromSlider(this.value);
        });

        $cameraDistance.on('input change', function () {
            applyCameraDistanceFromSlider(this.value);
        });
...
//        world.add(Physics.behavior('body-impulse-response'));
        ...
        world.subscribe('beforeRender', function (data) {
            ...
            if(primaryChainAtoms.length > 0) {
                ...
                renderer.camera.position.y = gCameraHeight;
        ...
        $('#3dcontainer').bind('mousewheel', function (e) {
            e.preventDefault();
            var sliderValue = parseFloat($cameraDistance.val()) || cameraDistanceToSliderValue(gCameraDistance);
            var step = 20;
            ...
            $cameraDistance.val(sliderValue);
            applyCameraDistanceFromSlider(sliderValue);
            return false;
        });
